...
namespace tf3d::generators
{
    ...
    public:
        bool LoadFromFile(const std::string &path, std::string *error = nullptr)
        {
            ...
            if (!file.is_open())
                return fail("Could not open noise algorithm index: " + path);
            ...
            if (algorithms.empty())
                return fail("Noise algorithm index cannot be empty: " + path);
            ...
            for (size_t index = 0; index < algorithms.size(); ++index) {
                ...
                if (!IsValidShaderToken(token))
                    return fail("Invalid shader token for noise algorithm: " + token);
        ...
        int MaxValue() const
        {
            return m_Entries.empty() ? -1 : m_Entries.back().value;
        }
        ...
        const std::vector<NoiseAlgorithmEntry> &Entries() const
        {
            return m_Entries;
        }
...
using tf3d::generators::NoiseAlgorithmCatalog;
using tf3d::generators::NoiseAlgorithmEntry;
