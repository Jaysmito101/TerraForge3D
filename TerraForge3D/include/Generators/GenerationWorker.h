...
public:
	...
	inline bool HasContext() const { return m_Window != nullptr; }
	inline bool IsRunning() const { return m_Running.load(std::memory_order_acquire); }
	inline bool IsRequestPending() const { return m_RequestPending.load(std::memory_order_acquire); }
	inline bool IsCompleted() const { return m_Completed.load(std::memory_order_acquire); }
