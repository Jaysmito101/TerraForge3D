...
public:
	...
	inline bool IsReady() const { return m_HasData.load(std::memory_order_acquire); }
	inline float GetSampleRadius() const { return m_SampleRadius; }
	inline void SetSampleRadius(float radius) { m_SampleRadius = radius > 0.0f ? radius : 1.0f; }
