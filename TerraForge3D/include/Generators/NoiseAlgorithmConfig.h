...
inline bool ApplyTF3DNoiseAlgorithmMetadata(nlohmann::json& config, const TF3DNoiseAlgorithmCatalog& catalog)
{
	...
	for (auto& parameter : config["Params"])
	{
		...
		if (parameter.value("Name", "") == "NoiseAlgorithm")
		{
			...
			{
				int defaultValue = 0;
				if (!catalog.TryGetValue(parameter["Default"].get<std::string>(), defaultValue)) return false;
		...
		for (const auto& conditionalValue : parameter["ConditionalValues"])
		{
			...
			else if (conditionalValue.is_string())
			{
				...
				if (!catalog.TryGetValue(conditionalValue.get<std::string>(), algorithmValue)) return false;
