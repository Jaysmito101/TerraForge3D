...
namespace tf3d::data
{
    ...
    namespace
    {
        std::filesystem::path ResolveUserConfigDirectory()
        {
            ...
            if (home && *home)
                return std::filesystem::path(home) / ".terraforge3d";
    ...
    bool ConfigManager::GetString(const std::string &section, const std::string &key, std::string &value) const
    {
        ...
        if (sectionIt == m_Config.end() || !sectionIt->is_object())
            return false;

        const auto valueIt = sectionIt->find(key);
        if (valueIt == sectionIt->end() || !valueIt->is_string())
            return false;
    ...
    bool ConfigManager::SetString(const std::string &section, const std::string &key, const std::string &value)
    {
        ...
        if (!sectionObject.is_object())
            sectionObject = nlohmann::json::object();
    ...
    bool ConfigManager::GetBool(const std::string &section, const std::string &key, bool &value) const
    {
        ...
        const auto valueIt = sectionIt->find(key);
        if (valueIt == sectionIt->end() || !valueIt->is_boolean())
            return false;
    ...
    {
        if (name == m_LastSavedThemeName && serializedStyle == m_LastSavedThemeData)
            return false;
