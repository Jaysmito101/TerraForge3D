...
{
	if (m_AccumulationRendererID != 0) glDeleteTextures(1, &m_AccumulationRendererID);
	if (m_RawRendererID != 0) glDeleteTextures(1, &m_RawRendererID);
	if (m_RendererID != 0) glDeleteTextures(1, &m_RendererID);
	if (m_WorkingRendererID != 0) glDeleteTextures(1, &m_WorkingRendererID);
	...
	int32_t resolution, int32_t targetSamples, int32_t samplesPerDispatch)
{
	...
	if (heightPyramid == nullptr || !heightPyramid->IsReady() ||
		m_Shader == nullptr || m_FilterShader == nullptr || terrainWorldSize <= 0.000001f)
	{
		return false;
	}
...

	{
		...
		if (m_WorkPending || m_WorkComplete) return false;
	}
	if (m_Worker == nullptr || !m_Worker->HasContext()) return false;
	EnsureTextures(resolution);
	if (!m_ResetPending && m_AccumulatedSamples >= targetSamples) return false;
...
void HeightfieldGICache::RunWorkerBuild()
{
	...
	glBindTexture(GL_TEXTURE_CUBE_MAP, work.skyRadianceRendererID >= 0
		? static_cast<uint32_t>(work.skyRadianceRendererID) : 0);
	glActiveTexture(GL_TEXTURE2);
	glBindTexture(GL_TEXTURE_CUBE_MAP, work.skyIrradianceRendererID >= 0
		? static_cast<uint32_t>(work.skyIrradianceRendererID) : 0);
	glActiveTexture(GL_TEXTURE3);
	glBindTexture(GL_TEXTURE_2D, work.terrainSelfShadowRendererID >= 0
		? static_cast<uint32_t>(work.terrainSelfShadowRendererID) : 0);
...
{
	if (m_Worker == nullptr || m_Worker->IsRunning() || m_Worker->IsRequestPending()) return;
...
{
	if (!m_IsReady || m_RendererID == 0) return;
	glActiveTexture(GL_TEXTURE0 + textureSlot);
