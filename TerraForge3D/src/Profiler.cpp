...
namespace
{
    ...
    void DrawHotPathList(const std::vector<PerformanceMonitor::FrameSnapshot> &frames)
    {
        ...
        std::sort(sortedPaths.begin(), sortedPaths.end(), [](const auto &left, const auto &right) {
            if (left.second.totalMs != right.second.totalMs)
                return left.second.totalMs > right.second.totalMs;
...
uint64_t PerformanceMonitor::BeginEvent(std::string_view key)
{
    ...
    if (threadNameIterator != m_ThreadNames.end())
        activeEvent.threadName = threadNameIterator->second;
...
void PerformanceMonitor::EndEvent(uint64_t eventId)
{
    ...
    if (activeIterator == m_ActiveEvents.end())
        return;
    ...
    if (stackIterator != m_ThreadStacks.end()) {
        ...
        if (eventIterator != stack.rend())
            stack.erase(std::next(eventIterator).base());
        if (stack.empty())
            m_ThreadStacks.erase(stackIterator);
    }
            ...
            if (ImGui::BeginTable("##PerformanceCallTree", 5,
                ...
                if (!m_GroupCallTreeByThread) {
                    ...
                    for (const FrameSnapshot &frame : snapshot.capturedFrames)
                        for (const EventSnapshot &event : frame.events)
                            AddCallTreeEvent(root, event);
                    SortCallTree(root);
                    for (const CallTreeNode &child : root.children)
                        DrawCallTreeNode(child, child.key);
                } else {
