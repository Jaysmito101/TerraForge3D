...
#define TF3D_HANDLE_EXCEPTION_MSG(x, message)            \
    {                                                    \
        try {                                            \
            x;                                           \
        } catch (const std::exception &e) {              \
            TF3D_LOG_ERROR("{}: {}", message, e.what()); \
        }                                                \
    }
...
{
    bool loaded          = false;
    std::string contents = ReadShaderSourceFile(filepath, &loaded);
    if (loaded)
