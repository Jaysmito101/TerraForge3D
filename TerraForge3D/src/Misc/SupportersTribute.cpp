...
namespace tf3d::misc
{
    ...
    void SupportersTribute::LoadstargazersData(nlohmann::json &data)
    {
        ...
        for (nlohmann::json item : data) {
            if (!item.is_object() || !item.contains("login") || !item.contains("node_id") || !item.contains("avatar_url"))
                continue;
            GitHubData st;
            st.name = item["login"];
    ...
    void SupportersTribute::LoadcontributorsData(nlohmann::json &data)
    {
        ...
        for (nlohmann::json item : data) {
            ...
            st.avatar = new Texture2D(GetExecutableDir() + PATH_SEPARATOR "Data" PATH_SEPARATOR "cache" PATH_SEPARATOR "github_avatars" PATH_SEPARATOR + st.name + "_" +
