        ...
        if (ImGui::BeginPopup("Choose Base Model##Main")) {
            if (ImGui::Button("Open From File")) {
                ...
                if (FileExists(path)) {
                    ...
                    if (tmp) {
                        delete m_AppState->mainModel;
                        m_AppState->mainModel = tmp;
                        tmp->isGeneratedPlane = false;
                        tmp->mesh->RecalculateNormals();
                        tmp->SetupMeshOnGPU();
                        tmp->UploadToGPU();
                    }
                }
            ...
            if (ImGui::BeginCombo("Gnerate Base Model##Choose Base Model Main", default_base_models[selected_item])) {
                for (int i = 0; i < IM_ARRAYSIZE(default_base_models); i++) {
                    ...
                    if (ImGui::Selectable(default_base_models[i], is_selected))
                        selected_item = i;
                    if (is_selected)
            ...
            if (selected_item == 0) {
                ...
                if (ImGui::DragInt("Segments##GenBaseTorus", &numSegments, 0.1f, 1, 100000))
                    numSegments = std::clamp(numSegments, 1, 100000);
                if (ImGui::DragInt("Rings##GenBaseTorus", &numRings, 0.1f, 1, 100000))
                    numRings = std::clamp(numRings, 1, 100000);
