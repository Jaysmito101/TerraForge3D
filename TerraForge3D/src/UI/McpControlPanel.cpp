...
namespace
{
	...
	UiJson VisibleParameters(const UiJson& parameters)
	{
		...
		if (visible.is_object()) visible.erase("_meta");
		return visible;
	}
	...
	McpCommandDisplay SummarizeCommand(const McpCommandLogEntry& command)
	{
		...
		if (command.method == "initialize")
		{
			...
			if (!clientVersion.empty()) display.detail += " " + clientVersion;
			return display;
		}
		...
		if (command.method == "tools/call")
		{
			...
			if (display.title == "Call tool: ") display.title = "Call tool";
		...
		if (command.method == "resources/read")
		{
			...
			if (!uri.empty()) display.detail = uri;
			return display;
		}

		if (command.method == "tools/list") display.title = "List tools";
		else if (command.method == "resources/list") display.title = "List resources";
		else if (command.method == "resources/templates/list") display.title = "List resource templates";
		else display.title = command.method;
...
void McpControlPanel::ShowSettings()
{
	...
	if (m_AppState && m_AppState->mcpServer) stats = m_AppState->mcpServer->GetStats();

	const char* runtimeStatus = "Unavailable";
	if (stats.running) runtimeStatus = "Running";
	else if (m_RuntimeEnabled) runtimeStatus = "Stopped";
	else runtimeStatus = "Disabled";
	...
	ImGui::SameLine();
	if (!m_CopyMessage.empty()) ImGui::TextDisabled("%s", m_CopyMessage.c_str());
	...
	if (!m_LastError.empty()) ImGui::TextColored(ImVec4(1.0f, 0.35f, 0.35f, 1.0f), "%s", m_LastError.c_str());
