...
bool ShowSeedSettings(const std::string &label, int *seed, std::vector<int> &historyStack)
{
    ...
    auto drawPanel = [&](const ImVec2 &min, const ImVec2 &max, bool hovered, bool active, bool selected, bool action) {
        ...
        if (selected || active)
            backgroundColor = action ? ImGuiCol_ButtonActive : ImGuiCol_FrameBgActive;
        else if (hovered)
            backgroundColor = action ? ImGuiCol_ButtonHovered : ImGuiCol_FrameBgHovered;
    ...
    if (valueHovered)
        ImGui::SetTooltip("Drag to adjust • Double-click to enter an exact value");
    if (valueHovered && ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left))
        ImGui::OpenPopup("##SeedEdit");
    ...
    if (isSaved)
        ...
        drawList->PathStroke(iconColor, true, 1.5f);
    if (saveHovered)
        ImGui::SetTooltip(isSaved ? "Remove this seed from saved seeds" : "Save this seed for quick reuse");
    if (ImGui::IsItemClicked(ImGuiMouseButton_Left)) {
        if (isSaved)
            historyStack.erase(savedSeed);
        else
            historyStack.push_back(*seed);
    }
    ...
    if (historyHovered)
        ImGui::SetTooltip("Open saved seed history");
    if (ImGui::IsItemClicked(ImGuiMouseButton_Left))
        ImGui::OpenPopup("##SeedHistoryPopup");
    ...
    if (ImGui::BeginPopup("##SeedEdit")) {
        ...
        if (ImGui::InputInt("##SeedEditValue", seed, 1, 10))
            changed = true;
    ...
    if (ImGui::BeginPopup("##SeedHistoryPopup")) {
        ...
        ImGui::Separator();
        if (ImGui::Button("Clear saved seeds") && !historyStack.empty())
            historyStack.clear();
        ImGui::EndPopup();
    }
