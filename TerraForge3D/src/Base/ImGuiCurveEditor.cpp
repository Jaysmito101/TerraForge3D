...
namespace ImGui
{
    ...
    {
        if (key == nullptr || v == nullptr || num <= 0 || dim <= 0)
            return;
        if (num == 1) {
    ...
    float CurveValueSmooth(float p, int maxpoints, const ImVec2 *points)
    {
        ...
        if (p <= points[0].x)
            return points[0].y;
        if (p >= points[maxpoints - 1].x)
            return points[maxpoints - 1].y;

        int rightIndex = 1;
        while (rightIndex < maxpoints && points[rightIndex].x < p)
            ++rightIndex;
        if (rightIndex >= maxpoints)
            return points[maxpoints - 1].y;

        const int leftIndex      = rightIndex - 1;
        const float segmentWidth = points[rightIndex].x - points[leftIndex].x;
        if (segmentWidth <= 0.000001f)
            return points[rightIndex].y;
        ...
        return a * points[previousIndex].y + b * points[leftIndex].y + c * points[rightIndex].y + d * points[nextIndex].y;
    }
    ...
    float CurveValue(float p, int maxpoints, const ImVec2 *points)
    {
        if (maxpoints < 2 || points == nullptr)
            return 0.0f;
        if (!std::isfinite(p) || p <= points[0].x)
            return points[0].y;
        if (p >= points[maxpoints - 1].x)
            return points[maxpoints - 1].y;
...
int Curve(const char *label, const ImVec2 &size, const int maxpoints, ImVec2 *points)
{
	...
	*/
    ...
    int Curve(const char *label, const ImVec2 &size, const int maxpoints, ImVec2 *points)
    {
        ...
        if (!std::isfinite(points[0].x) || points[0].x < 0.0f || !std::isfinite(points[1].x) || points[1].x < 0.0f)
            resetCurve();
        ...
        for (int index = 0; index < pointCount; ++index) {
            ...
            if (!std::isfinite(points[index].x))
                points[index].x = index / static_cast<float>(pointCount - 1);
            if (!std::isfinite(points[index].y))
                points[index].y = index / static_cast<float>(pointCount - 1);
        ...
        for (int index = 1; index < pointCount - 1; ++index) {
            ...
            if (std::abs(points[index].x - clampedX) > 0.000001f)
                modified = 1;
        ...
        if (hovered && ImGui::IsMouseClicked(ImGuiMouseButton_Left)) {
            ...
            if (closestPoint >= 0) {
                ...
                int insertIndex = 1;
                while (insertIndex < pointCount - 1 && points[insertIndex].x < position.x)
                    ++insertIndex;
                for (int index = pointCount; index > insertIndex; --index)
        ...
        if (hovered && ImGui::IsMouseClicked(ImGuiMouseButton_Right)) {
            ...
            if (closestPoint >= 0) {
                for (int index = closestPoint + 1; index < pointCount; ++index)
                    points[index - 1] = points[index];
                --pointCount;
                points[pointCount] = ImVec2(-1.0f, -1.0f);
                if (draggingCurveId == id) {
                    draggingCurveId    = 0;
        ...
        if (ImGui::Button("Flip##CurveFlip")) {
            for (int index = 0; index < pointCount; ++index)
                points[index].y = 1.0f - points[index].y;
            modified = 1;
        }
        ImGui::SameLine();
