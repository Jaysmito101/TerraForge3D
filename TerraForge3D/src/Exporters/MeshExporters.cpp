...
bool ExportManager::ExportMesh(std::string path, Mesh *mesh, int format)
{
    ...
    switch (format) {
        case 0:
            path += ".obj";
            return m_ObjExporter.Export(path, mesh, &m_ExportProgress);
        case 1:
            path += ".stl";
            return m_StlExporter.ExportASCII(path, mesh, &m_ExportProgress);
        case 2:
            path += ".stl";
            return m_StlExporter.ExportBinary(path, mesh, &m_ExportProgress);
        case 3:
            path += ".ply";
            return m_PlyExporter.ExportASCII(path, mesh, &m_ExportProgress);
        case 4:
            path += ".ply";
            return m_PlyExporter.ExportBinary(path, mesh, &m_ExportProgress);
        case 5:
            path += ".dae";
            return m_DaeExporter.Export(path, mesh, &m_ExportProgress);
        case 6:
            path += ".gltf";
            return m_GltfExporter.ExportGLTF(path, path + ".bin", mesh, &m_ExportProgress);
        default:
            return false;
    }
    return false;
...
void ExportManager::ExportMeshCurrentTile(std::string path, bool *exporting, int format, bool updateWorkerUpdation)
{
    ...
    auto worker        = ([path, format, exporting, meshClone, heightMapData, this]() -> void {
        ...
        if (!ExportMesh(path, ApplyMeshTransform(meshClone, heightMapData), format))
            SetStatusMessage("Failed to export : " + path);
        else
            SetStatusMessage("");
