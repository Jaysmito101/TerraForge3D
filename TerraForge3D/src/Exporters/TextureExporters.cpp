...
namespace tf3d::exporters
{
    ...
    bool ExportManager::ExportHeightmapTexture(std::string path, float *data, int format, int bitDepth, int resolution)
    {
        ...
        switch (format) {
            case 0:
                path += ".png";
                return m_PngTextureExporter.ExportHeightmap(path, data, bitDepth, resolution, &m_ExportProgress);
            case 1:
                path += ".webp";
                return m_WebpTextureExporter.ExportHeightmap(path, data, bitDepth, resolution, &m_ExportProgress);
            case 2:
                path += ".raw";
                return m_RawTextureExporter.ExportHeightmap(path, data, bitDepth, resolution, &m_ExportProgress);
            case 3:
                path += ".exr";
                return m_ExrTextureExporter.ExportHeightmap(path, data, bitDepth, resolution, &m_ExportProgress);
            default:
                return false;
        }
        return false;
    ...
    void ExportManager::ExportTextureCurrentTile(std::string path, int format, int bitDepth, bool *exporting)
    {
        ...
        {
            TF3D_PROFILE_SCOPE_DOMAIN("export/texture/readback", PerformanceMonitor::Domain::Wait);
        ...
        auto worker = ([path, format, exporting, bitDepth, heightMapData, this]() -> void {
            ...
            TF3D_PROFILE_SCOPE_DOMAIN("export/texture/write", PerformanceMonitor::Domain::Io);

            this->SetStatusMessage("Exporting : " + path);
            m_ExportProgress = 0.01f;

            if (!ExportHeightmapTexture(path, ApplyHeightmapTextureTransform(heightMapData), format, bitDepth, m_AppState->mainMap.tileResolution))
                this->SetStatusMessage("Failed to export : " + path);
            else
