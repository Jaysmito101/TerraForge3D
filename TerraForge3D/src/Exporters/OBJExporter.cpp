...
bool OBJExporter::Export(const std::string &path, Mesh *mesh, float *progress)
{
    ...
    *progress = 0.0f;
    ...
    if (!this->WriteHeader(out_strm, mesh, progress))
        return false;
    *progress = 0.05f;
    if (!this->WriteVertices(out_strm, mesh, progress))
        return false;
    *progress = 0.2f;
    if (!this->WriteNormals(out_strm, mesh, progress))
        return false;
    *progress = 0.4f;
    if (!this->WriteTextureCoordinates(out_strm, mesh, progress))
        return false;
    *progress = 0.6f;
    if (!this->WriteFaces(out_strm, mesh, progress))
        return false;
    *progress                 = 0.8f;
