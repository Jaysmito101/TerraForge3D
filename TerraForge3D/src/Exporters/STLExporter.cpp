...
bool STLExporter::ExportBinary(const std::string& path, Mesh* mesh, float* progress)
{
	...
	uint32_t facet_count = (uint32_t)(mesh->indexCount / 3);
	writer.Write(facet_count);
	float tmpp = 1.0f / mesh->indexCount;
	for (int i = 0; i < mesh->indexCount; i += 3)
	{
		const auto& pa = mesh->vert[mesh->indices[i + 0]].position;
		const auto& pb = mesh->vert[mesh->indices[i + 1]].position;
		const auto& pc = mesh->vert[mesh->indices[i + 2]].position;
