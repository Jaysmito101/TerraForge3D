...
namespace tf3d::exporters
{
    ...
    void GLTFExporter::PrepareMeta(Mesh *mesh, const std::string &bin_path)
    {
        ...
        tmp_js["version"]   = "2.0";
        gltf_js["asset"]    = tmp_js;
        tmp_js              = nlohmann::json();

        gltf_js["scene"] = 0;

        tmp2_js["name"]  = "Scene";
        tmp2_js["nodes"] = {0};
        tmp_js.push_back(tmp2_js);
        tmp2_js           = nlohmann::json();
        gltf_js["scenes"] = tmp_js;
        tmp_js            = nlohmann::json();
        ...
        tmp_js.push_back(tmp2_js);
        tmp2_js          = nlohmann::json();
        gltf_js["nodes"] = tmp_js;
        tmp_js           = nlohmann::json();

        tmp3_js["POSITION"]   = 0;
        tmp3_js["NORMAL"]     = 1;
        tmp3_js["TEXCOORD_0"] = 2;
        tmp2_js["attributes"] = tmp3_js;
        tmp3_js               = nlohmann::json();
        tmp2_js["indices"]    = 3;
        tmp_js["primitives"]  = nlohmann::json();
        tmp_js["primitives"].push_back(tmp2_js);
        tmp2_js           = nlohmann::json();
        tmp_js["name"]    = "MainOBJ";
        gltf_js["meshes"] = nlohmann::json();
        gltf_js["meshes"].push_back(tmp_js);
        tmp_js = nlohmann::json();
        ...
        tmp_js["type"]          = "VEC3";
        gltf_js["accessors"].push_back(tmp_js);
        tmp_js = nlohmann::json();
        ...
        gltf_js["bufferViews"].push_back(tmp_js);
        tmp_js = nlohmann::json();
        ...
        tmp_js["uri"]        = bin_path;
        gltf_js["buffers"].push_back(tmp_js);
        tmp_js = nlohmann::json();
