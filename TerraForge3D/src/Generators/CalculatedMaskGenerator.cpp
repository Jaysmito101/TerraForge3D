...
namespace
{
	...
	std::string InjectMaskModeDefines(const std::string& source, const nlohmann::json& metadata)
	{
		...
		if (firstLineEnd == std::string::npos) return source;
...
{
	if (m_Inspector == nullptr || !m_Inspector->HasVariable("MaskType")) return static_cast<int>(m_Settings.type);
...
{
	if (m_Inspector == nullptr || !m_Metadata.contains("Types") || !m_Metadata["Types"].is_array()) return false;
	if (typeIndex < 0 || typeIndex >= static_cast<int>(CalculatedMaskType::Count) || typeIndex >= static_cast<int>(m_Metadata["Types"].size())) return false;

	const auto& types = m_Metadata["Types"];
	std::vector<std::string> typeNames;
	typeNames.reserve(types.size());
	for (const auto& type : types) typeNames.push_back(type.value("Name", type.value("ID", "Mask")));
...
{
	if (m_AppState == nullptr) return false;
...
{
	if (id.empty() || !m_Metadata.contains("Types") || !m_Metadata["Types"].is_array()) return -1;
	for (size_t typeIndex = 0; typeIndex < m_Metadata["Types"].size(); ++typeIndex)
	{
		if (m_Metadata["Types"][typeIndex].value("ID", "") == id) return static_cast<int>(typeIndex);
	}
	return -1;
}
...
void CalculatedMaskGenerator::SyncSettingsFromInspector()
{
	...
	if (m_Inspector->HasVariable("Range"))
	{
		...
		if (range.GetType() == CustomInspectorValueType_Vector2)
		{
			...
			if (value.x > value.y) m_Inspector->GetVariable("Range").SetVector2(glm::vec2(m_Settings.minimum, m_Settings.maximum));
		}
		else
		{
	...
	if (m_Inspector->HasVariable("EdgeFeather")) m_Settings.softness = m_Inspector->GetVariable("EdgeFeather").GetFloat();
	if (m_Inspector->HasVariable("EdgeSoftness")) m_Settings.softness = m_Inspector->GetVariable("EdgeSoftness").GetFloat();
	if (m_Inspector->HasVariable("Direction")) m_Settings.angle = m_Inspector->GetVariable("Direction").GetFloat();
...
{
	if (!m_MetadataLoaded || m_Inspector == nullptr) return false;
...
SerializerNode CalculatedMaskGenerator::Save() const
{
	...
	if (m_Inspector != nullptr) node->SetChildNode("Inspector", m_Inspector->SaveData());
	return node;
}

void CalculatedMaskGenerator::Load(SerializerNode data)
{
	if (data == nullptr || m_Inspector == nullptr) return;
	...
	if (typeIndex < 0 && data->HasKey("MaskMode"))
	{
		...
		if (m_Metadata.contains("Types") && m_Metadata["Types"].is_array())
		{
			for (size_t candidateIndex = 0; candidateIndex < m_Metadata["Types"].size(); ++candidateIndex)
				if (m_Metadata["Types"][candidateIndex].value("Mode", -1) == shaderMode) typeIndex = static_cast<int>(candidateIndex);
		}
	}
	if (typeIndex < 0)
