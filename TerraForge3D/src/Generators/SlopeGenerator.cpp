...
{
	if (m_Texture == nullptr || resolution <= 0) return;
	if (m_Texture->GetWidth() == resolution && m_Texture->GetHeight() == resolution) return;
...
{
	if (heightmap == nullptr || m_Shader == nullptr || m_Texture == nullptr || resolution <= 0) return false;
	Resize(resolution);
