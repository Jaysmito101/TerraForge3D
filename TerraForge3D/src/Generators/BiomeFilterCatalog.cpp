...
bool BiomeFilterCatalog::Reload()
{
	...
	{
		if (error) break;
		if (!entry.is_regular_file() || entry.path().filename() != "filter.json") continue;
	...
	{
		if (left->GetCategory() != right->GetCategory()) return left->GetCategory() < right->GetCategory();
...
std::shared_ptr<BiomeFilterDefinition> BiomeFilterCatalog::FindByID(const std::string& id) const
{
	for (const auto& definition : m_Definitions)
		if (definition->GetID() == id) return definition;
	return nullptr;
}
