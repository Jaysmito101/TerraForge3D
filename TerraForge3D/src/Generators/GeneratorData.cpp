...
void GeneratorData::Resize(size_t size) 
{
	...
	if (resolution <= 0 || static_cast<size_t>(resolution) * resolution != texelCount) return;
	m_Size = size;
...
void GeneratorData::SetData(const void* data, size_t offset, size_t size)
{
	...
	if (m_Size < size || offset != 0 || size != m_Size) return;
	if (m_Size == 0) Resize(size);
	glBindTexture(GL_TEXTURE_2D, m_RendererID);
...
bool GeneratorData::CopyTo(const GeneratorData* other)
{
	...
	if (m_Resolution != other->m_Resolution || m_InternalFormat != other->m_InternalFormat) return false;
...
{
	if (offset != 0 || size != m_Size) return;
	glBindTexture(GL_TEXTURE_2D, m_RendererID);
...
void GeneratorData::SetDefaultStorage(GeneratorDataStorage storage)
{
	...
	for (auto* instance : s_Instances) instance->SetStorage(storage);
}

GeneratorDataStorage GeneratorData::GetDefaultStorage() { return s_DefaultStorage; }
