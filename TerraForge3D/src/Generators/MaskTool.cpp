...
{
	if (m_ExternalGeneratedTexture == nullptr) return false;
...
void MaskTool::RasterizeStrokes()
{
	...
	auto appendStroke = [&](const MaskStroke& stroke)
	{
		...
		for (const auto& point : stroke.points) points.emplace_back(point.x, point.y, 0.0f, 0.0f);
	};

	for (const auto& stroke : m_Strokes) appendStroke(stroke);
	if (m_HasActiveStroke) appendStroke(m_ActiveStroke);
...
{
	if (!m_HasActiveStroke) return;
	if (!m_ActiveStroke.points.empty()) m_Strokes.emplace_back(std::move(m_ActiveStroke));
	m_ActiveStroke = MaskStroke{};
	m_HasActiveStroke = false;
	RasterizeStrokes();
}
...
bool MaskTool::ShowPaintedSettings()
{
	...
	if (m_IsEditing)
	{
		...
		if (ImGui::SliderFloat("Brush size", &m_DrawSettings.m_BrushSize, 0.005f, 2.0f)) changed = true;
		if (ImGui::SliderFloat("Falloff", &m_DrawSettings.m_BrushFalloff, 0.0f, 1.0f)) changed = true;
		if (ImGui::SliderFloat("Strength", &m_DrawSettings.m_BrushStrength, 0.0f, 1.0f)) changed = true;
...
bool MaskTool::ShowSettings(bool showViewportMask)
{
	...
	else
	{
		if (m_ExternalGeneratedTexture != nullptr)
		{
			...
			if (ImGui::Button("Manually Edit")) CopyGeneratedMaskToPainted();
		}
		else
		{
	...
	if (activeViewport)
	{
		...
		if (m_PreviewMode == MaskPreviewMode::Painted && m_IsEditing)
		{
			...
			{
				if (m_DrawSettings.m_BrushMode != 1) m_PreviousBrushMode = m_DrawSettings.m_BrushMode;
