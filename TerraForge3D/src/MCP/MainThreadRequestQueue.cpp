...
MainThreadRequestQueue::MainThreadRequestQueue()
	...
	{
		if (!task) return McpResult::Failure("invalid_task", "MCP request did not contain executable work");
		{
			std::lock_guard<std::mutex> lock(mutex);
			if (stopped) return McpResult::Failure("shutdown", "TerraForge3D is shutting down");
		}
	...
	{
		if (std::this_thread::get_id() != ownerThread) return 0;
	...
	void MainThreadRequestQueue::Shutdown()
	{
		...
		{
			std::lock_guard<std::mutex> lock(mutex);
			if (stopped && requests.empty()) return;
			stopped = true;
