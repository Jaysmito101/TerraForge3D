...
namespace tf3d::texture_store
{
    ...
    void TextureStore::VerifyTextureThumbs()
    {
        ...
        for (auto it = textureDatabaseJ.begin(); it != textureDatabaseJ.end(); it++) {
            if (!FileExists(GetExecutableDir() + PATH_SEPARATOR "Data" PATH_SEPARATOR "cache" PATH_SEPARATOR "texture_thumbnails" PATH_SEPARATOR + it.key() + ".png")) {
                ...
                DownloadFile("https://cdn.polyhaven.com", "/asset_img/thumbs/" + it.key() + ".png?width=100&height=100", GetExecutableDir() + PATH_SEPARATOR "Data" PATH_SEPARATOR "cache" PATH_SEPARATOR "texture_thumbnails" PATH_SEPARATOR + it.key() + ".png");
            }
        }
    }
    ...
    void TextureStore::LoadTextureDatabase()
    {
        ...
        for (auto it = textureDatabaseJ.begin(); it != textureDatabaseJ.end(); it++) {
            ...
            item.name           = it.key();
            item.thumbnailPath  = GetExecutableDir() + PATH_SEPARATOR "Data" PATH_SEPARATOR "cache" PATH_SEPARATOR "texture_thumbnails" PATH_SEPARATOR + item.name + ".png";
    ...
    void TextureStore::DownloadTexture(int id, int res)
    {
        ...
        std::string baseDir = GetExecutableDir() + PATH_SEPARATOR "Data" PATH_SEPARATOR "textures" PATH_SEPARATOR + textureStoreItems[id].name;
        MkDir(baseDir);
