...
layout(std430, binding = 3) readonly buffer StrokeRangesBuffer
{
	ivec4 u_StrokeRanges[]; // point start, point count, unused, unused
};

layout(std430, binding = 4) readonly buffer StrokePointsBuffer
{
...
void main()
{
	...
	imageStore(u_MaskTexture, coordinate, vec4(value, 0.0, 0.0, 1.0));
}
